...
public class AutoUpdater {
	...
	public enum UpdateResult {
		...
		private static final Map<Integer, AutoUpdater.UpdateResult> valueList = new HashMap<Integer, AutoUpdater.UpdateResult>();
		...
		public static AutoUpdater.UpdateResult getResult(int value) {
			return valueList.get(value);
		}

		static {
			for (AutoUpdater.UpdateResult result : AutoUpdater.UpdateResult.values()) {
	...
	public enum UpdateType {
		...
		private static final Map<Integer, AutoUpdater.UpdateType> valueList = new HashMap<Integer, AutoUpdater.UpdateType>();
		private final int value;
		...
		public static AutoUpdater.UpdateType getResult(int value) {
			return valueList.get(value);
		}

		static {
			for (AutoUpdater.UpdateType result : AutoUpdater.UpdateType.values()) {
				valueList.put(result.value, result);
			}
		}
	}
	 ...
	 *            updates in console
	 */
	public AutoUpdater(Plugin plugin, String slug, File file, UpdateType type,
			boolean announce) {
		this.plugin = plugin;
		this.type = type;
		...
		try {
			...
			Log.warn("The author of this plugin has misconfigured their Auto Update system");
			Log.warn("The project slug added ('" + slug + "') is invalid, and does not exist on dev.bukkit.org");
			result = AutoUpdater.UpdateResult.FAIL_BADSLUG; // Bad slug! Bad!
		}
		if (url != null) {
			// Obtain the results of the project's file feed
			readFeed();
		...
		try {
			...
			while ((line = buff.readLine()) != null) {
				...
				// Search for size
					...
					String size = line.replaceAll("<dd>", "")
							.replaceAll("</dd>", "");
	...
	private boolean versionCheck(String title) {
		if (type != UpdateType.NO_VERSION_CHECK) {
			//NoItem - Remove the build type and number from the version
			...
			} else { // The file's name did not contain the string 'vVersion'
				Log.warn("The author of this plugin has misconfigured their Auto Update system");
				Log.warn("Files uploaded to BukkitDev should contain the version number, seperated from the name by a 'v', such as PluginName v1.0");
				Log.warn("Please notify the author (" + plugin.getDescription().getAuthors().get(0) + ") of this error.");
